...
def detect_gestures(fingers_left, fingers_right, lmList_left=None, lmList_right=None):
    ...
    if fingers_left:
        # Debug output - uncomment to see finger values
        # print(f"Left hand fingers: {fingers_left}")
        ...
        # Open hand (all fingers up) for unlock
        if fingers_left[0] == 1 and fingers_left[1] == 1 and fingers_left[2] == 1 and fingers_left[3] == 1 and fingers_left[4] == 1:
    ...
    if fingers_right:
        # Debug output - uncomment to see finger values
        # print(f"Right hand fingers: {fingers_right}")
        ...
        # PLAY - Open hand (all fingers extended)
        if fingers_right[0] == 1 and fingers_right[1] == 1 and fingers_right[2] == 1 and fingers_right[3] == 1 and fingers_right[4] == 1:
            gestures["play"] = True
    ...
    # Debug - Print active gestures
    active_gestures = [name for name, active in gestures.items() if active]
    if active_gestures:
...
def calculate_volume_from_hand(thumb_tip, index_tip):
    ...
    # Map vertical position to volume (higher hand = higher volume)
    # Adjusted range to use full frame height and reversed mapping
    base_vol = np.interp(hand_height, [50, h-50], [100, 0])  # Use most of frame height, leaving margins
    
    # Map pinch distance to a volume adjustment (fine-tuning)
    # Reduced adjustment range to prevent volume getting stuck at high values
    vol_adjustment = np.interp(distance, [15, 100], [-5, 5])
    
    # Combine for final volume
    vol = base_vol + vol_adjustment
    vol = np.clip(vol, 0, 100)  # Ensure volume stays within 0-100 range
...
while not _quit.is_set():
        ...
        for idx, hand in enumerate(multi_handedness):
            ...
            if xList and yList:
                ...
                for finger_id in range(1, 5):
                    ...
                    if lmList[tip_id][2] < lmList[tip_id-2][2]:  # Fixed tipId to tip_id
                        fingers.append(1)
                    else:
                        fingers.append(0)
    ...
    if controlMode == "VOLUME":
        ...
        cv2.putText(img, "Open Hand: Play", (w-220, 50), 
